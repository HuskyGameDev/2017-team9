...
public partial class AkBasePathGetter
{
	...
	public static string GetFullSoundBankPath()
	{
		// Get full path of base path
 		...
 		var fullBasePath = AkInitializer.GetBasePath();
#else
		var fullBasePath = System.IO.Path.Combine(UnityEngine.Application.streamingAssetsPath, AkInitializer.GetBasePath());
#endif
	...
	public static string GetPlatformBasePathEditor(string platformName)
	{
		...
		if (string.IsNullOrEmpty(SoundBankDest))
			UnityEngine.Debug.LogWarning("WwiseUnity: The SoundBank folder could not be determined.");
		else
		{
			try
			{
